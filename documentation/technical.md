...
response = requests.post(
    "http://localhost:8088/api/tokenize",
    json={
        "input_text": "What is the capital of France?",
        "output_text": "The capital of France is Paris."
    }
...
### Notes
- `method` is "bpe" to indicate the tokenization technique
- Token counts are based on GPT-2-style BPE
- Either `input_text` or `output_text` can be omitted if only one needs to be tokenized
