...
PROVIDER_USERNAME=$2
MAX_RETRIES=5
BASE_URL="http://auth:8081"  # Using internal container URL
...
# Add balances for both users
echo "\nAdding balances for users..."
balance_sql="INSERT INTO balances (id, user_id, available_amount, held_amount, created_at, updated_at) 
             VALUES 
             (gen_random_uuid(), '$consumer_user_id', 100, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
             (gen_random_uuid(), '$provider_user_id', 100, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);"

docker exec -i cockroachdb cockroach sql --insecure -d inferoute -e "$balance_sql"
...
echo "\nBALANCES:"
echo "  Added 100 available balance for both users"
echo "----------------------------------------" 